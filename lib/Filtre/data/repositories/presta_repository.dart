...
class PrestaRepository {
  ...
  /// Récupère tous les types de prestataires au format modèle
  Future<List<PrestaTypeModel>> getPrestaTypes() async {
    try {
      _logger.d('Fetching prestataire types and converting to model objects');
      
      // Utiliser la méthode existante qui récupère les données de Supabase
      final List<Map<String, dynamic>> typesData = await getPrestaTypesAsMap();
      
      // Convertir en liste de PrestaTypeModel
      final List<PrestaTypeModel> result = typesData.map((data) => 
        PrestaTypeModel.fromMap(data)
      ).toList();
      
      _logger.d('Converted ${result.length} prestataire types to models');
      return result;
    } catch (e) {
      _logger.e('Error converting prestataire types to models: $e');
        ...
        return [
          {
            ...
            'description': 'Description du prestataire de type $typeId',
            'region': 'Paris',
          ...
          },
          {
            ...
            'description': 'Autre description pour le type $typeId',
            'region': 'Lyon',
